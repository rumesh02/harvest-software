...
const MerchantSidebar = () => {
  ...
  return (
    <div style={{ width: 320, background: "#f9f9f9", height: "100vh", padding: "24px", borderRadius: "10px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
      {/* Top Section - Logo and Menu */}
      <div>
        {/* Logo and Title */}
        <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "24px" }}>
          <img src="/images/logo.png" alt="Logo" width={50} />
          <h2 style={{ fontSize: "20px", fontWeight: "bold", margin: 0 }}>Farm-to-Market</h2>
        </div>
        ...
        <List>
          {menuItems.map((item) => (
            ...
            >
              <ListItemIcon sx={{ 
                color: location.pathname === item.path ? "#D97706" : "#6B7280",
                minWidth: "48px"
              }}>
                {item.icon}
              </ListItemIcon>
      ...
      <div>
        ...
        <div style={{ textAlign: "center", marginBottom: "20px" }}>
          <Badge overlap="circular" badgeContent={"Merchant"} color="primary">
            <Avatar src={isAuthenticated ? user.picture : "/images/placeholder.svg"} sx={{ width: 64, height: 64, margin: "0 auto" }} />
          </Badge>
          <h4 style={{ marginTop: 12, fontSize: "18px", fontWeight: "bold" }}>{isAuthenticated ? user.name : "Guest"}</h4>
        </div>
        ...
        <List>
          ...
          >
            <ListItemIcon sx={{ minWidth: "48px" }}><Settings /></ListItemIcon>
            <ListItemText 
              primary="Settings" 
          ...
          >
            <ListItemIcon sx={{ minWidth: "48px" }}><ExitToApp /></ListItemIcon>
            <ListItemText 
              primary="Log Out" 
              primaryTypographyProps={{ fontSize: "16px" }}
            />
