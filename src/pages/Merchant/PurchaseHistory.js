...
import { Box, Typography, Card, CardMedia, CardContent, Button, Grid, Dialog, DialogTitle, DialogContent, DialogActions, TextField } from "@mui/material";
import ReviewDialog from "../../components/ReviewDialog"; // Adjust the path if needed
...
const purchases = [
  ...
  {
    id: 2,
    image: "https://www.tastingtable.com/img/gallery/why-are-red-onions-purple/intro-1644158494.jpg",
    date: "2025-02-10",
    farmer: "Sunimal Perera",
    price: "Rs. 5000",
    quantity: "10 kg",
...
const PurchaseHistory = () => {
  ...
  const handleReviewSubmit = (review) => {
    alert(`Review submitted for ${selectedFarmer}!\nRating: ${review.rating}\nComment: ${review.comment}`);
  };

  const handleFindVehicle = (location) => {
    if (!location) {
      alert("Location not available for this item.");
      return;
    }
    alert(`Finding vehicles for location: ${location}`);
  };
  ...
  const handleLocationUpdate = (location) => {
    setCurrentLocation(location); // Update the location from LocateMe component
  };
    ...
    <Box sx={{ padding: "20px" }}>
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 1, p: 2 }}>
                ...
                <Box sx={{ display: "flex", justifyContent: "space-between", gap: 1 }}>
                  <Button
                    ...
                    color="primary"
                    sx={{ flex: 1 }}
                    onClick={() => handleFindVehicle(purchase.location)}
                  >
                    Find Vehicle
                  </Button>
                  <Button
      ...
      <Dialog open={locationDialogOpen} onClose={() => setLocationDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          />
          <LocateMe onLocationUpdate={handleLocationUpdate} /> {/* Use LocateMe component */}
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setLocationDialogOpen(false)} color="secondary">
            Cancel
          </Button>
          <Button
            onClick={() => {
              alert(`Street: ${streetName}, City: ${city}, Location: ${currentLocation || "Not located"}`);
